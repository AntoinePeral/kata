...
function groupByCity(data: TemperatureData[]): Record<string, number[]> {
...
function getCityWithHighestAverage(data: TemperatureData[]): string {
...
function getMaxAverageTemp  (inputs: TemperatureData[]){

  const cityTemps: Record<string, number[]> = inputs.reduce((acc, input) => {
    if (!acc[input.city]) {
      acc[input.city] = [];
    }
 ...
 const cityAvgTemps: {city:string, avgTemp:number}[] = Object.entries(cityTemps).map(([city, temps]) => ({
    city,
    avgTemp: temps.reduce((sum, temp) => sum + temp, 0) / temps.length,
    }));
    

  const maxCity = cityAvgTemps.reduce((max, cityData) =>
    cityData.avgTemp > max.avgTemp ? cityData : max
  );
